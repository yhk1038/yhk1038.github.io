...
[Xgboost 논문](https://arxiv.org/abs/1603.02754) 리뷰 및 코드를 작성한 내용입니다. 논문의 전체를 리뷰하진 않고 특정 부분만 작성했습니다
...
- $$\gamma T+\frac{1}{2}\lambda ||w||^{2}$$ : L2 norm of leaf scores
...
$$\widetilde{L}^{(t)} = \sum_{i=1}^{n}[g_{i}f_{t}(x_{i})+\frac{1}{2}h_{i}f_{t}^{2}(x_{i})]+\Omega(f_{t})\qquad(3)$$

- $$\Omega$$를 풀면

$$\widetilde{L}^{(t)} = \sum_{i=1}^{n}[g_{i}f_{t}(x_{i})+\frac{1}{2}h_{i}f_{t}^{2}(x_{i})]+\gamma T+\frac{1}{2}\lambda\sum_{j=1}^{T}w_{j}^{2}\\
